function new_integrate_points() {
    var url = 'dashboard/integracion_puntos_rango/load';
    location.href = site + url;
}

function edit_points(id) {
    var url = 'dashboard/integracion_puntos_rango/load/' + id;
    location.href = site + url;
}

function cancel_points() {
    var url = 'dashboard/integracion_puntos_rango';
    location.href = site + url;
}
...
function delete_points(id){
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                type: "post",
                url: site+"dashboard/integracion_puntos_rango/delete",
                dataType: "json",
                data: {id : id},
                success: function (data) {
                    if (data.status == true) {
                        ...
                        window.setTimeout(function () {
                            window.location = site + "dashboard/integracion_puntos_rango";
                        }, 1500);
                    } else {
                        Swal.fire({
                            position: 'top-end',
...
function integration_active_points(){
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: site + "dashboard/integracion_puntos_rango/active_points",
                method: "POST",
                data: oData,
                contentType: false,
                cache: false,
